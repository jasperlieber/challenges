    ...
    public Long resume() throws Exception
    {
        ...
        while (running && output == null)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                // NEW CHANGE: input is zero if non existent
                // if (inputIndex >= inputs.size())
                // throw new Exception("inputIndex=" + inputIndex
                // + " but inputs.size=" + inputs.size());

                Long inputPoll = inputs.poll();
                long input = inputPoll == null ? 0 : inputPoll;

//                System.out.println("input=" + input + " inputs=" + inputs);

                addr = po.set(1, input);
                Log.out("Input: " + input + " = " + mem.get(addr) + " into pos "
                        + addr);
                ip += 2;
                break;
            case 4:
            ...
            default:
                throw new Exception("for pos " + ip
                        + " -- unknown opcode value " + mem.get(ip)
