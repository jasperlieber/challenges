...
public class Day03Part2
{
    ...
    private void doit() throws Exception
    {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(filename)))
        {
            ...
            while ((line = br.readLine()) != null)
            {
                ...
                {
                    segs2 = addSegs(line);
                    int steps = findEarliestCross(segs1, segs2);
                    System.out.println("least steps = " + steps);
                }
    ...
    private int findEarliestCross(WireSegs segs1, WireSegs segs2)
            throws Exception
    {
        int leastSteps = -1;
        ...
        for (WireSeg seg1 : segs1)
        {
            ...
            for (WireSeg seg2 : segs2)
            {
                ...
                if (cross != null && cross.nonZero())
                {
                    int steps = len1 - seg1.len() + seg1.distTo(cross) + len2 - seg2.len() + seg2.distTo(cross);
                    
                    if (leastSteps == -1 || steps < leastSteps)
                        leastSteps = steps;
                }
                System.out.println("Cross of " + seg1 + " & " + seg2 + " is "
                        + cross + ", leastSteps = " + leastSteps);
            }
        }
    ...
    private class WireSeg extends Pair<Coord, Coord>
    {
        ...
        public int distTo(Coord cross)
        {
            int x1 = this.getFirst().getFirst();
            int y1 = this.getFirst().getSecond();
            int x2 = cross.getFirst();
            int y2 = cross.getSecond();
        ...
        public Coord intersect(WireSeg seg2) throws Exception
        {
            ...
            if (denom == 0)
            {
                ...
                {
                    System.out
                            .println("coincident segs: " + this + " & " + seg2);
                }
                else
                {
            ...
            if (0f <= t && t <= 1f && 0f <= u && u <= 1f)
            {
                ...
                System.out.println("cross = " + cross + " t = " + t + " u = "
                        + u + " denom = " + denom);

            }
            else
            {
                System.out.println("no cross, t = " + t + " u = " + u
                        + " denom = " + denom);
            }
