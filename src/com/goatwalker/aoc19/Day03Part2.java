...
public class Day03Part2 {
  ...
  private int findEarliestCross(WireSegs segs1, WireSegs segs2) throws Exception {
    int leastSteps = -1;
    ...
    for (WireSeg seg1 : segs1) {
      ...
      for (WireSeg seg2 : segs2) {
        ...
        if (cross != null && cross.nonZero()) {
          int steps =
              len1 - seg1.len() + seg1.distTo(cross) + len2 - seg2.len() + seg2.distTo(cross);

          if (leastSteps == -1 || steps < leastSteps)
            leastSteps = steps;
        }
        System.out.println(
            "Cross of " + seg1 + " & " + seg2 + " is " + cross + ", leastSteps = " + leastSteps);
      }
    }
